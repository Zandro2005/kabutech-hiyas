...
function generateYieldExcelReport() {
    ...
    const buildRow = (key, entry) => {
        ...
        const row = [key, Number(kg), entry.harvestCount, Number(avg)];
        ...
        // Add total grams at the end
        row.push(entry.totalGrams);
        return row;
    };
    ...
    // Apply column widths
    const wscols = [{wch:15}, {wch:18}, {wch:15}, {wch:22}, ...racks.map(() => ({wch: 16})), {wch:15}];
    wsDaily['!cols'] = wscols;
