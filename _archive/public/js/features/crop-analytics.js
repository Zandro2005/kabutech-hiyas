...
function computeHarvestSummary(racks) {
    ...
    rackData.sort((a, b) => (a.name || "").localeCompare(b.name || "", undefined, { numeric: true, sensitivity: 'base' }));
...
function computeRackUtilization(racks) {
    ...
    rackStats.sort((a, b) => (a.name || "").localeCompare(b.name || "", undefined, { numeric: true, sensitivity: 'base' }));
...
function openWidgetModal(modalId) {
    ...
    if (modalId === "modal-log-harvest") {
        ...
        const updateTotal = () => {
            const active = parseInt(document.getElementById("capacity-active").value) || 0;
            const empty = parseInt(document.getElementById("capacity-empty").value) || 0;
            const rackId = parseInt(select.value);
            const rack = state.growBatches.find(r => r.id === rackId);
            let preserved = 0;
            if (rack && rack.bags) {
                const existingBags = Object.values(rack.bags).filter(b => b != null);
                preserved = existingBags.filter(b => b.status === "Contaminated" || b.status === "Replaced").length;
            }
            document.getElementById("capacity-total-display").textContent = active + empty + preserved;
        };

        document.getElementById("capacity-active").oninput = updateTotal;
        document.getElementById("capacity-empty").oninput = updateTotal;
        ...
        select.onchange = () => {
            ...
            if (rack && rack.bags) {
                ...
                document.getElementById("capacity-active").value = 0;
                document.getElementById("capacity-empty").value = 0;
            }
            updateTotal();
        };
        // trigger initial
...
function populateRackDropdown(selectId) {
    ...
    let addedCount = 0;
    const sortedRacks = [...state.growBatches].sort((a, b) => 
        (a.rack || "").localeCompare(b.rack || "", undefined, { numeric: true, sensitivity: 'base' })
    );

    sortedRacks.forEach(rack => {
        if (selectId === "harvest-rack-select" || selectId === "contam-rack-select") {
            const bags = Object.values(rack.bags || {}).filter(b => b != null);
    ...
    if (addedCount === 0) {
        select.innerHTML = `<option value="">No eligible racks</option>`;
    }
}
